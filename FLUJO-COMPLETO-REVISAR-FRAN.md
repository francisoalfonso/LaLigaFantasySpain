# 📋 FLUJO E2E COMPLETO: De `npm run veo3:test-nano-banana` hasta `test-history.html`

**Documento**: Análisis exhaustivo del flujo completo Nano Banana → VEO3
**Fecha**: 2025-10-11
**Objetivo**: Documentar CADA PASO del proceso desde comando inicial hasta visualización en test-history.html
**Total pasos**: 383 pasos detallados
...
### Paso 4
Script carga variables de entorno (.env) mediante `require('dotenv').config()` (línea 18)
...
### Paso 15
Construye URL del endpoint: `http://localhost:3000/api/veo3/generate-with-nano-banana`
...
### Paso 18
Si servidor no responde → error ECONNREFUSED → mensaje "Servidor no está corriendo. Ejecuta: npm run dev"
...
### Paso 19
Express server (`backend/server.js`) recibe POST request en `/api/veo3/generate-with-nano-banana`
...
### Paso 23
Logger registra inicio: `"🎨 Generando video con Nano Banana: chollo, preset: chollo_viral"`
...
### Paso 36
UnifiedScriptGenerator (`backend/services/veo3/unifiedScriptGenerator.js`) ejecuta `generateUnifiedScript()`
...
### Paso 43
Llama a `cinematicSystem.getFullProgression('chollo', ['curiosidad', 'autoridad', 'urgencia'])`
...
[
  ...
  { ...outro, cinematography: { name: 'medium close-up', description: '...' } }
]
```
...
### Paso 47
Llama a `nanoBananaVeo3Integrator.generateImagesFromScript(scriptSegments, options)` (línea 1423)

### Paso 48
NanoBananaVeo3Integrator (`backend/services/veo3/nanoBananaVeo3Integrator.js`) recibe 3 segmentos
...
### Paso 51
Construye prompt contextualizado llamando a `buildContextualImagePrompt(segment)` (línea 174)
...
- Busca descripción específica: "Close-up shot of face and shoulders only, tight framing showing facial details..."
...
### Paso 54
Llama a `nanoBananaClient.generateContextualImage(imagePrompt, 'close-up', options)` (línea 182)

### Paso 55
NanoBananaClient (`backend/services/nanoBanana/nanoBananaClient.js`) ejecuta request a KIE.ai API
...
### Paso 61
Sube imagen a Supabase Storage llamando a `supabaseFrameUploader.uploadFrame()` (línea 193)
...
- Devuelve: `https://ixfowlkuypnfbrwawxlx.supabase.co/storage/v1/object/sign/ana-images/video-frames/seg1-intro-*.png?token=...`
...
### Paso 111
De vuelta en veo3.js, logger: `"✅ 3 imágenes contextualizadas generadas (costo: $0.060)"`
...
### Paso 121
Construye prompt VEO3 usando `promptBuilder.buildNanoBananaPrompt(segment.dialogue)` (línea 1464)

### Paso 122
PromptBuilder.buildNanoBananaPrompt() (`backend/services/veo3/promptBuilder.js` líneas 279-293):
```javascript
...
### Paso 124
Logger: `"🖼️ Prompt Nano Banana (intro): 27 chars"` (debería decir words, pero dice chars en el log)
...
### Paso 126
Llama a `veo3Client.generateCompleteVideo(nanoBananaPrompt, veo3Options)` (línea 1478)

### Paso 127
VEO3Client (`backend/services/veo3/veo3Client.js`) ejecuta `generateCompleteVideo()`
...
### Paso 134
VEO3Client ejecuta `getStatus(taskId)` → GET a `https://kie.ai/api/get_result?taskId=...`
...
### Paso 142
Ejecuta `axios.get(videoResult.url, { responseType: 'arraybuffer' })` (línea 1485)
...
### Paso 146
Escribe archivo con `fs.promises.writeFile(localPath, response.data)` (línea 1489)
...
### Paso 150
Escribe progress.json con `fs.promises.writeFile(progressFile, JSON.stringify(progressData, null, 2))`
...
### Paso 153
Ejecuta `setTimeout()` con delay de 30000ms (30 segundos) - **cooling period VEO3**
...
- Prompt basado en diálogo segment2: "Los números son espectaculares, misters... este jugador dobla su valor en puntos Fantasy. Está volando en rendimiento." (**17 palabras** vs 10 anteriores)
...
### Paso 261
VideoConcatenator (`backend/services/veo3/videoConcatenator.js`) recibe 3 paths + options:
```javascript
{
...
### Paso 271
Resultado final: `output/veo3/ana-concatenated-1760124824380.mp4` (21.8 segundos)
...
### Paso 351
Escribe test-report.json en sessionDir: `fs.writeFileSync(reportPath, JSON.stringify(report))`
...
- **segment_1_*.mp4** (video segmento 1, 1.85 MB)
- **segment_2_*.mp4** (video segmento 2, 1.65 MB)
- **segment_3_*.mp4** (video segmento 3, 1.75 MB)
...
### Paso 376
Navegador carga video desde `http://localhost:3000/output/veo3/ana-concatenated-*.mp4`
...
- **Segmento 2 (7s)**: Ana con Medium Shot, diálogo middle (**17 palabras**, sin silencios extraños)
...
### Paso 382
Servidor guarda feedback en `data/instagram-versions/video-${sessionId}-metadata.json`
...
1. **Diálogo Segment2 extendido**: 10 palabras → **17 palabras** (6.8s audio vs 4s anterior)
...
6. **Persistencia inmediata**: progress.json actualizado después de cada segmento
...
**Última actualización**: Paso 383 completado

...
const payload = {
    ...
    presenter: 'carlos', // ✅ Test con Carlos González (seed: 30002)
    customScript: [
        {
            role: 'intro',
            duration: 8,
            dialogue:
                'Misters, acabo de ver el video de Fantasy Football Pro España sobre el delantero polaco... y hay datos cruciales que NO os están contando. ¡Escuchad esto!',
            emotion: 'mysterious'
        },
        {
            role: 'middle',
            duration: 8,
            dialogue:
                'Los números reales son: 5 goles en 5 jornadas, 1 asistencia, y un rating de 6.8... nada comparado con lo que están vendiendo por ahí. ¡Reflexionad!',
            emotion: 'confident'
        },
        {
            role: 'outro',
            duration: 8,
            dialogue:
                'Ahora vosotros decidís: confiar en el hype de un 11M o en datos reales. Yo ya os lo he dicho. ¡No os quedéis atrás!',
            emotion: 'urgent'
        }
    ]
};
...
axios
    .post('http://localhost:3000/api/veo3/prepare-session', payload, {
    ...
    .then(response => {
        ...
        if (response.data.success && response.data.data?.sessionId) {
            ...
            console.log('\n📌 Siguiente paso:');
            console.log(
                `   POST /api/veo3/generate-segment con sessionId=${response.data.data.sessionId}`
            );
        }
